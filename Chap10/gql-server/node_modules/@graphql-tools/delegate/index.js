    ...
    await Promise.all([...delegationMap.entries()].map(async ([subschema, selectionSet]) => {
        ...
        if (resolver) {
            ...
            if (source instanceof Error || source == null) {
                const fieldNodeResponseKeyMap = utils.collectFields(schema, EMPTY_OBJECT, EMPTY_OBJECT, type, selectionSet, new Map(), new Set());
                const nullResult = {};
...
function resolveExternalObject(type, object, unpathedErrors, subschema, context, info, skipTypeMerging) {
    ...
    const typeName = graphql.isAbstractType(type) ? object.__typename : type.name;
